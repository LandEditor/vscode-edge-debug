...
export class EdgeDebugAdapter extends ChromeDebugAdapter {
	...
	private _launchAdapter(args?: any): Promise<any> {
		...
		logger.log(
			`Launching adapter at: '${adapterExePath}', ${JSON.stringify(arguments)})`,
		);
		// Check exists
		if (!fs.existsSync(adapterExePath)) {
			if (utils.getPlatform() == utils.Platform.Windows) {
				return utils.errP(
					`No Edge Diagnostics Adapter was found. Install the Edge Diagnostics Adapter (https://github.com/Microsoft/edge-diagnostics-adapter) and specify a valid 'adapterExecutable' path`,
				);
			} else {
				return utils.errP(
					`Edge debugging is only supported on Windows 10.`,
				);
			}
		}
		...
		return utils.getURL(`http://127.0.0.1:${args.port}/json/version`).then(
			(jsonResponse: any) => {
				try {
					...
					return utils.errP(
						`Sever for ${targetBrowser} already listening on :9222`,
					);
				} catch (ex) {
					return utils.errP(
						`Sever already listening on :9222 returned ${ex}`,
					);
				}
			},
			(error) => {
	...
	public constructor(
		...
		debugSession?: ChromeDebugSession,
	) {
